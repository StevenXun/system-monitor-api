...
from sys_api.utils import build_response
...
@router.get("/disk")
def get_disk(min_usage: int = Query(0, ge=0, le=100), top_n: int | None = Query(None, ge=1)):
    logger.info(f"received disk request with min_usage={min_usage}, top_n={top_n}")
